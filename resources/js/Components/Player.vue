<template lang="pug">
footer.flex.flex-1.items-center.bg-gray-dark.p-3
  //- Song info
  .flex.gap-6.flex-1
    template(v-if="currentSong != null")
      div
        img.hidden.md_block.h-full(:src="playlist.songs[currentSong].image" alt="Song image" width="56")
      .flex.gap-6.items-center
        div
          a(href="#").no-underline.text-white.block {{ playlist.songs[currentSong].name }}
          a(href="#").no-underline.text-gray-lightest.text-sm {{ playlist.songs[currentSong].artist }}
        div
          button.border-none.bg-none(role="switch" aria-label="Guardar en biblioteca")
            svg.fill-current.text-white(role="img" height="16" width="16" viewBox="0 0 16 16")
  ...
  .flex-1
    .flex.flex-col.gap-1
      ...
      .flex.items-center.gap-2.text-white
        span.relative.top-px.leading-none 0:00
        .bg-gray-light.w-full.h-1.rounded.relative
          .bg-gray-lightest.rounded.h-full.inline-block.absolute.w-full
        span.relative.top-px.leading-none {{ currentSong ? playlist.songs[currentSong].duration : '0:00' }}
